use bytes::BytesMut;
use futures::{Sink, Stream, ready, FutureExt};
use std::{
    marker::PhantomData,
    pin::Pin,
    task::{Context, Poll},
    ...
    Out: Unpin + Send,
{
    ...
    fn poll_next(mut self: Pin<&mut Self>, cx: &mut Context<'_>) -> Poll<Option<Self::Item>> {
        ...
        // ready! 宏会在 Pending 时直接 return Pending ，而在 Ready 时，返回 Ready 的值
        ready!(Box::pin(fut).poll_unpin(cx))?;

        // 拿到一个 frame 的数据，把 buffer 合并回去
        self.rbuf.unsplit(rest);

        // 调用 decode_frame 来获取解包后的数据
